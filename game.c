...
void RunInputLoop(char **board, int rows, int cols, int snakeSize)
{
    ...
    int **previousMoves = (int **)malloc(sizeof(int *) * (rows * cols));
    ...
    initPreviousMoves(previousMoves, rows, cols, snakeSize);
    ...
    while (status != 0)
    {
        ...
        else
        {
            ...
            printStateInfoInJson(board, rows, cols, tail, head, direction, previousMoves, snakeSize);
            awaitingInput();
        }
        enableBuffer();
        printf("\n");
    ...
    freePreviousMoves(previousMoves, rows, cols);
